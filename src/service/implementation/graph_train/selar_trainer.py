...
class SELARTrainerImpl(Trainer):
    
    def __init__(
        self,
        learner_model: torch.nn,
        task_specific_model: torch.nn,
        weighting_model: torch.nn,
        hint_model: torch.nn,
        ...
        self.learner_optimizer = torch.optim.Adam(self.learner_model.params(), lr=self.lr, weight_decay=1e-4)
        self.hint_optimizer = torch.optim.Adam(self.hint_model.params(), lr=self.lr, weight_decay=1e-4)
    ...
    def fit(self) -> None:
        ...
        for epoch in tqdm.tqdm(range(self.epochs), colour='green', desc='SELf-supervised Auxiliary leaRning (SELAR) with Meta-path Prediction'):
