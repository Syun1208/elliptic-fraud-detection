...
class DataNetWork:
    def __init__(
        ...
        self.fraud_dict = dict(
            zip(
                df_features["transid"].map(self.map_id),
                df_features["class"]
                )
            )
    ...
    def _set_up_network_info(self):
        nodes = self.df_features['transid']
        map_id = {j:i for i,j in enumerate(nodes)} #add map to have uniform mapping over the three networks
        if self.directed:
            edges = self.df_edges[['current_transid', 'next_transid']] 
        else:
            edges_direct = self.df_edges[['current_transid', 'next_transid']]
            edges_rev = edges_direct[['next_transid', 'current_transid']]
            edges_rev.columns = ['current_transid', 'next_transid']
            edges = pd.concat([edges_direct, edges_rev])
        
        nodes = nodes.map(map_id)
        edges.current_transid = edges.current_transid.map(map_id)
        edges.next_transid = edges.next_transid.map(map_id)
    ...
    def get_network_torch(self):
        ...
        features = self.df_features[self.df_features.columns.drop(['transid', 'class'])]
